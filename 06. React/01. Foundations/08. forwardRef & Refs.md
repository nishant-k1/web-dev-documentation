...
const Input = forwardRef<HTMLInputElement, { label: string; type?: string }>(
...
```jsx
const ValidatedInput = forwardRef(({ label, validate, ...props }, ref) => {
  const inputRef = useRef();
...
class FancyButton extends React.Component {
  ...
  render() {
    return (
      <button ref={(el) => (this.buttonRef = el)}>{this.props.children}</button>
    );
  }
}
