...
**useMemo does execute your function.**
**It returns a memoized version of the result of the executed callback function (first param)**
👉 It executes the function you pass and memoizes the returned result, not the function itself.
...
function SalesChart({ salesData, timeRange }) {
  const chartData = useMemo(() => {
    ...
    const filtered = salesData.filter(
      (sale) => sale.date >= timeRange.start && sale.date <= timeRange.end,
    );
...
function Component() {
  ...
  return <Child data={expensiveResult} onClick={handleClick} />;
}
```
...
function Component({ largeArray }) {
  const sorted = useMemo(() => largeArray.sort((a, b) => a - b), [largeArray]);
}
```
