# Error Boundaries in React (Just like we have try cath block for catching errors in functions, equivalent to that we have error boundaries to catch rendering errors in react components)

In JavaScript/TypeScript, we use try...catch to catch exceptions in imperative code (regular function logic, async operations, event handlers, etc.).
In React, error boundaries serve a very similar purpose, but for the declarative rendering phase of components.
They are React's way of implementing "try-catch for the render tree".

| Aspect                | try...catch (JS/TS)                          | Error Boundaries (React)                                                                  |
| --------------------- | -------------------------------------------- | ----------------------------------------------------------------------------------------- |
| What they catch       | Runtime errors in imperative code            | Errors during rendering, constructors, and (legacy) lifecycle methods                     |
| Where they work       | Inside functions, async code, event handlers | In the component tree (child components)                                                  |
| What they don't catch | —                                            | Event handlers, useEffect/useLayoutEffect, async code (promises, setTimeout, etc.)        |
| Recovery mechanism    | Catch → handle / recover / rethrow           | Catch → show fallback UI + log error                                                      |
| Granularity           | Per block of code                            | Per subtree (wrap components / sections / routes)                                         |
| Implemented as        | Language syntax                              | Special class component (or library like react-error-boundary)                            |
| Modern usage          | Still the only way for most logic            | Still class-based in core React (2025–2026), but libraries provide hook-friendly versions |

🎯 Key Concept to Lock In

Error Boundary → protects render phase
try-catch + state → protects effect phase
Together → build resilient UI
...
- Error Boundaries catch errors that occur
  ...
  - `They do not catch errors in:`
    ...
    - useEffect callbacks (For async operations inside useEffect, we should handle errors explicitly using try-catch and log them appropriately to monitoring tools)
    - Promise rejections
  ...
  Error Boundaries only protect the render phase, not the effect phase.
