...
- props (React has different behaviors for props depending on whether you're using regular components or optimized components)
  ...
  2. `React.memo (Function Components)` → Uses shallowEqual: Wrapped in React.memo, the component uses shallowEqual to compare props.
     ...
     - It compares each prop at the first level.
     - For primitive values → compares by value.
     - For objects/functions/arrays → compares by reference.

  **But you can see no shallow comparison being actually done instead either value comparison or reference, the why we say shallow comparison?**
  - Because not all props are objects. There is no reference comparison happening for primitives. So saying “reference comparison” is incomplete. Shallow comparison covers both cases.
    - It compares each prop at the first level.
    - For primitive values → compares by value.
    - For objects/functions/arrays → compares by reference.
    - It does not perform deep comparison.

  **React does NOT do this:**
  - Compare entire props object reference
  - If different → then compare each prop
  - That is not how React.memo works.
  - React does not care about the combined props object (whole props wrapper object) reference. The props object always gets new reference for each render so if reference comparisons will take place for the props object then child memo wrapped component will always render as shallow comparison will fail always.

  **React.memo does NOT:**
  - Compare entire props object wrapper, `because props wrapper object is recreated every render`. It compares each prop individually.
  - Compare nested fields
  - Go deeper than first-level prop keys
  - It only compares each prop’s value using Object.is.
