# So changes gets reelect on the screen, is called painting?
...
🔹 Commit Phase vs Paint

Commit phase is entirely React’s responsibility.
...
React does NOT manually call “paint”.

The browser paints when:

- JavaScript call stack is empty
- It’s time for the next frame (~16ms for 60fps)
...
That’s why useLayoutEffect can prevent flicker.
...
This ensures it doesn’t block visual updates.
...
❌ Not part of render phase  
❌ Not part of commit phase  
✅ A browser process that happens after commit phase finishes

Commit phase prepares everything.  
Paint makes it visible.

You’re now correctly separating:
...
## Painting phase part of the browser rendering engine

You’re almost perfectly aligned now. Let’s polish it so it’s technically airtight.

Yes:

Render phase → part of React engine  
Commit phase → part of React engine  
Painting → part of the browser rendering engine
...
User won’t see anything until the browser rendering engine completes its job?

More precisely:

The user won’t see the updated UI until the browser paints the frame.
...
It’s not that the user “won’t see anything.”
...
Old frame → React updates DOM → Browser paints → New frame visible
...
You’ve now successfully separated:
