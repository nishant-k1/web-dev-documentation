# Render Phase vs Commit Phase

Render Phase → React calculates what should change (virtual tree updated)  
Commit Phase → React applies those changes to the real DOM

That’s correct at a high level.

But let’s clean it up precisely.

---

🔹 Render Phase

What actually happens:

- React calls your component functions
- Hooks execute
- Dependencies are compared
- A new Fiber tree (work-in-progress tree) is built

React figures out:

- What changed
- What needs updating

Important:

❌ No real DOM mutation happens here  
❌ Nothing visible changes on screen  
✅ It’s pure calculation

This is often loosely called “virtual DOM update”, but technically React uses Fiber, not the old-style virtual DOM object.

---

🔹 Commit Phase

Now React:

- Applies changes to the real DOM
- Runs useLayoutEffect
- Browser paints
- Runs useEffect

This is where real visible change happens.

---

🔹 So Your Statement

Render Phase = virtual DOM gets updated  
Commit Phase = real DOM gets updated

✅ Conceptually correct

🔎 Technically more precise:

Render Phase = Fiber tree recalculated  
Commit Phase = DOM mutations + effect execution

---

🔹 One Important Nuance

Render phase can be:

- Paused
- Interrupted
- Aborted
- Re-run

Commit phase:

- Cannot be interrupted
- Always runs synchronously

That’s why React keeps them separate.

🔹 Clean Final Mental Model

Render Phase → "What should the UI look like?"
Commit Phase → "Make the UI actually look like that."

---

🔹 React Has Two Phases (Not Two Hook Categories)

React’s architecture has:

Render Phase  
Commit Phase

Hooks align with these phases.

---

🔹 1️⃣ Render-Phase Hooks

These run completely during render.

They:

- Must be pure
- Cannot cause side effects
- Execute immediately

Examples:

- useState
- useReducer
- useMemo
- useCallback
- useRef
- useContext

All of these:

- Execute during render
- Do not wait for commit
- Do not use the effect queue

Even useState is render-phase.  
Calling setState schedules another render — but the hook itself runs during render.

---

🔹 2️⃣ Commit-Phase Hooks (Effect Hooks)

These register side effects.

They:

- Are checked during render
- But execute during commit

Examples:

- useEffect → runs after paint
- useLayoutEffect → runs before paint
- useInsertionEffect → runs even earlier (for CSS-in-JS libraries)

These use:

- Effect tags
- Effect linked lists
- Commit-phase scheduling

🔹 So Architecturally, Yes

You can mentally classify hooks into:

| Category           | Hooks                                | When Callback Runs |
| ------------------ | ------------------------------------ | ------------------ |
| Render Phase Hooks | useMemo, useCallback, useState, etc. | During render      |
| Commit Phase Hooks | useEffect, useLayoutEffect           | During commit      |

But React documentation doesn’t officially call them that.

It’s an internal mental model.

🔹 Even More Precise Breakdown

If we go deeper, there are actually three practical categories:

---

1️⃣ State Hooks

Store data → trigger render

- useState
- useReducer

---

2️⃣ Memoization Hooks

Optimize calculations

- useMemo
- useCallback

---

3️⃣ Effect Hooks

Handle side effects

- useEffect
- useLayoutEffect
- useInsertionEffect

Only the third category participates in commit timing.

---

🔹 Important Subtlety

All hooks are called during render.

Only effect hooks defer execution of their callback.

That’s the key architectural difference.
