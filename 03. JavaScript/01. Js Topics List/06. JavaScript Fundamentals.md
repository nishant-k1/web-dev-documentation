...
## 11. Execution Context, Scope, and Closures
...
## 12. Asynchronous Operations Handling Tools

- **Promises**: `.then()`, `.catch()`, `.finally()`
- **Async/Await**: `async function() { await promise; }`

**Note**: ECMAScript provides the tools to handle async behavior: Promise, async/await, etc. But it does not provide the sources of async events like timers or network requests — that's the job of the host environment (browser, Node.js, etc.).
