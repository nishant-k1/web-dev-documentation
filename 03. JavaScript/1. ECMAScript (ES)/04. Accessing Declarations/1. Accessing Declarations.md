...
> 📖 **For detailed internal mechanisms:** See [10. Execution Context, Hoisting, Scope and Closures](<../10. Execution Context, Hoisting, Scope and Closures/>) for deep technical understanding of how JavaScript engine implements scope, hoisting, and closures.
...
**In lexical scoping:** `inner()` looks for `x` where it was **defined** (global scope) → "global"
**In dynamic scoping:** `inner()` would look for `x` where it was **called** from (outer scope) → "outer"
...
function outer() {
  ...
  inner();
  console.log(outerVar); // ✅ Accessible (same scope)
  // console.log(innerVar); // ❌ Error: not accessible (inner scope)
}

outer();
// console.log(outerVar); // ❌ Error: not accessible (function scope)
```
