...
1. **Creates Variable Environment:**
   ...
   - Function declarations are **initialized** with their full function body
   - `arguments` object is created (in function execution contexts)

2. **Creates Lexical Environment:**

   - Component structure is created
   - `let` and `const` declarations are **allocated but NOT initialized** (TDZ - Temporal Dead Zone)
...
3. **Creates this Binding:**
   - `this` binding is created and its value is **determined**
   - Global Context: `this` = global object (window/global)
   - Function Context: `this` = determined by how function is called
...
> 📖 **For the complete lifecycle flow and call stack behavior, see [04. Execution Context Life Cycle and Call Stack](./04.%20Execution%20Context%20Life%20Cycle%20and%20Call%20Stack.md).**
...
- Stores variable bindings for `let` and `const` declarations.
- Points to the Lexical Environment of the parent (outer) scope, forming a scope chain.
- Determines scope and enables closures.
- **Note:** Function declarations are NOT stored here—they go in the Variable Environment.

## 2. Variable Environment

- It is the part of an execution context responsible for storing variables and function declarations created during the creation (memory/hoisting) phase.

- The Variable Environment is similar to the Lexical Environment but is specifically responsible for storing:
  - `var` declarations.
...
**Note:** In modern JavaScript engines, the `Lexical Environment` and `Variable Environment` may share the same underlying structure for efficiency, but conceptually they remain separate. According to the ECMAScript specification:
...
```js
var funcExpr = function () {}; // funcExpr → Variable Environment
let arrowFunc = () => {}; // arrowFunc → Lexical Environment
const arrowFunc2 = () => {}; // arrowFunc2 → Lexical Environment
```

**Class Declarations:**

```js
class MyClass {} // MyClass → Lexical Environment (TDZ)
let MyClass2 = class {}; // MyClass2 → Lexical Environment
var MyClass3 = class {}; // MyClass3 → Variable Environment
```
...
```js
function test(a, b) {
  // a, b → Variable Environment
  var x = 10; // x → Variable Environment
}
```
...
```js
import { something } from "./module"; // something → Lexical Environment (TDZ)
import * as utils from "./utils"; // utils → Lexical Environment (TDZ)
```
...
3. **Rule of Thumb:**
   - If it's a **declaration** (not an expression), check the keyword:
     - `var` or `function` → Variable Environment
     - `let`, `const`, `class`, `import` → Lexical Environment
   - If it's an **expression** (function/arrow/class expression), follow the variable it's assigned to:
     - Assigned to `var` → Variable Environment
     - Assigned to `let`/`const` → Lexical Environment

**Why doesn’t let/const go in Variable Environment? Because ES6 split storage:**

1. `let` and `const` live in Lexical Environment (LE)
2. `var` and function declarations live in Variable Environment (VE)

This is why TDZ (Temporal Dead Zone) only affects `let`/`const` — not `var`.

### 🔥 Critical: Each Function Call Gets Its OWN Execution Context
...
- This is why closures work — each closure captures its own execution context (both Variable Environment and Lexical Environment)
...
**Note:** Closures capture the entire execution context (both Variable Environment and Lexical Environment), not just one or the other. The example above shows `let secret` (Lexical Environment) being captured, demonstrating that closures access both environments.

Think of it like lockers:

- `locker1` → Locker #101 with its own key (captures its own execution context)
- `locker2` → Locker #102 with its own key (captures its own execution context)
...
> 📖 **Deep Dive:** See [03.1. this Binding](./03.1.%20this%20Binding.md) for detailed examples and interview traps.
