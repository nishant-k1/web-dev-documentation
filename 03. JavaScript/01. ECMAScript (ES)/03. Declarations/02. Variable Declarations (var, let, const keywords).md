...
## let vs const vs var in classic loops and modern loops
...
**A classic for loop requires reassigning the loop variable, so const can’t be used. But in for…of and for…in, each iteration creates a new binding, so const works.**
