...
> 📖 **Detailed Coverage:** [Hoisting - TDZ Deep Dive](<../11. Execution Context, Hoisting, Scope and Closures/04.1. Hoisting.md#temporal-dead-zone-tdz---deep-dive>)
...
| ---------------- | ------- | ------------------------------ |
| `var`            | ❌ No   | ✅ Yes (but `undefined`)       |
| `let`            | ✅ Yes  | ❌ No (ReferenceError)         |
| `const`          | ✅ Yes  | ❌ No (ReferenceError)         |
| `class`          | ✅ Yes  | ❌ No (ReferenceError)         |
| `function`       | ❌ No   | ✅ Yes (fully hoisted)         |
| `import`         | ✅ Yes  | ❌ No (ReferenceError)         |
...
```javascript
// ✅ Works - earlier parameter is initialized
function works(a, b = a) {
  console.log(a, b);
}
works(5); // 5, 5

// ❌ Fails - later parameter is in TDZ
...
for (let i = 0; i < 3; i++) {
  ...
  setTimeout(() => {
    console.log(i); // ✅ Works - TDZ ended for this iteration's i
  }, 100);
}
```
