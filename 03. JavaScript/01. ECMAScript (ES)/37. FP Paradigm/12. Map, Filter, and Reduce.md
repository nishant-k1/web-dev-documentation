# FP Patterns: Map, Filter, and Reduce
...
👉 `map`, `filter`, `reduce` are supposed to be used with pure functions, i.e., no mutation should take place inside the callback function of those.
...
- But `reduce()` is the most commonly impure because developers often mutate:
  - external state
  - accumulator object
...
If you try to "filter" using `map()`, you'll end with:
...
> Pure map() filter (hacky - not recommended)
...
```js
const arr = [1, 2, 3, 4];
arr.reduceRight((acc, val) => acc - val, 0); // 0 - 4 - 3 - 2 - 1 = -10
arr.reduce((acc, val) => acc - val, 0); // 0 - 1 - 2 - 3 - 4 = -10
```
...
- `filter()` selects elements based on condition, returns shorter array
- `reduce()` accumulates to single value, most flexible
- All three are non-mutating (don't modify original array)
- Purity depends on the callback function you provide
- `reduce()` can implement both `map()` and `filter()`, but use specific methods for clarity
- `reduceRight()` processes array from right to left
