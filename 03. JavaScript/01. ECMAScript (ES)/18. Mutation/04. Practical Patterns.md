...
### ❌ Mutating (React Anti-Pattern - DON'T DO THIS)

```js
function updateUser(state, newAge) {
  state.users[0].age = newAge; // ❌ Mutates original state
  return state; // React won't detect change (same reference)
}
```

### ✅ Immutable Update (React Pattern - CORRECT)
...
const slice = createSlice({
  ...
  reducers: {
    addItem: (state, action) => {
      state.push(action.payload); // ✅ Looks like mutation, but Immer handles it immutably
    },
  },
});
```
...
function processData(data) {
  ...
  local.sort(); // ✅ Mutate local copy (fine, not shared)
  return local;
}
```
...
function buildConfig() {
  ...
  config.apiUrl = "https://api.example.com"; // ✅ Mutation OK
  config.timeout = 5000; // ✅ Mutation OK
...
function processLargeArray(arr) {
  ...
  arr.sort(); // ✅ OK if arr is local and won't be reused
  return arr;
}
```
...
items.sort(); // ✅ OK if items is local
items.reverse(); // ✅ OK if items is local
```
...
📖 See: [Copying](../../17.%20Copying/) - Copying techniques
📖 See: [Comparing](../../16.%20Comparing/) - Comparison methods
