...
`String` is a **built-in constructor** (and, like all functions in JS, an object). Use it as a constructor with `new String()` or via literals `""` / `''`; use static methods (e.g. `String.fromCharCode`, `String.fromCodePoint`) on the constructor itself.
