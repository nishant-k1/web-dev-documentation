...
`Object` is a **built-in constructor** (and, like all functions in JS, an object). Use it as a constructor with `new Object()` or via the literal `{}`; use its static methods (e.g. `Object.keys`, `Object.assign`) on the constructor itself.
...
## 🔷 3.5. Property Accessors (Getters/Setters)
...
| --------------------- | -------------- | ----------------- | ----------------- | ------------------------- |
| **Normal Object**     | ✅ Yes         | ✅ Yes            | ✅ Yes            | Fully mutable             |
| **preventExtensions** | ❌ No          | ✅ Yes            | ✅ Yes            | Prevents new properties   |
| **seal**              | ❌ No          | ❌ No             | ✅ Yes            | Prevents structure change |
| **freeze**            | ❌ No          | ❌ No             | ❌ No             | Fully immutable           |
...
obj1.city = "NYC"; // ✅ Can add
delete obj1.age; // ✅ Can delete
obj1.name = "Jane"; // ✅ Can modify

// preventExtensions - Can't add, but can delete and modify
let obj2 = { name: "John", age: 30 };
Object.preventExtensions(obj2);
obj2.city = "NYC"; // ❌ Cannot add (silently fails or throws in strict mode)
delete obj2.age; // ✅ Can delete
obj2.name = "Jane"; // ✅ Can modify

// seal - Can't add or delete, but can modify
let obj3 = { name: "John", age: 30 };
Object.seal(obj3);
obj3.city = "NYC"; // ❌ Cannot add
delete obj3.age; // ❌ Cannot delete (silently fails or throws in strict mode)
obj3.name = "Jane"; // ✅ Can modify

// freeze - Can't add, delete, or modify (fully immutable)
let obj4 = { name: "John", age: 30 };
Object.freeze(obj4);
obj4.city = "NYC"; // ❌ Cannot add
delete obj4.age; // ❌ Cannot delete
obj4.name = "Jane"; // ❌ Cannot modify (silently fails or throws in strict mode)
```
...
1. **Shallow Only:** All three methods work **shallowly** - nested objects are not affected:
   ...
   Object.freeze(obj);
   obj.nested.value = 20; // ✅ Still works! (nested object not frozen)
   obj.nested = {}; // ❌ Cannot replace (top level frozen)
   ```
...
## 🧠 Interview Highlights & Common Questions
...
// Deep clone
const deep1 = structuredClone(obj); // ✅ Modern (ES2022)
const deep2 = JSON.parse(JSON.stringify(obj)); // ⚠️ Limitations
...
📖 For comprehensive cloning methods, see [15. Copying](../../17.%20Copying/)
...
### 🔸 How to merge two objects deeply?
...
```js
// Shallow comparison
JSON.stringify(obj1) === JSON.stringify(obj2); // ⚠️ Order matters
...
### 🔸 How to remove undefined/null properties?
...
### 🔸 How to get all keys including non-enumerable?
...
Object.hasOwn(obj, "a"); // true ✅ (safer, newer ES2022)
...
📖 For comprehensive cloning guide, see [15. Copying](../../17.%20Copying/)
