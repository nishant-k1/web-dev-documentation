...
4. **Has a lifecycle** - Creation Phase → Execution Phase
...
2. **Execution Context (Runtime):**
   - When `outer()` is called → Function Execution Context is created
   - When `inner()` is called → Another Function Execution Context is created
...
> 📖 **For detailed Execution Context internals:** See [11. Execution Context, Hoisting, Scope and Closures](<../11. Execution Context, Hoisting, Scope and Closures/>) for comprehensive coverage of Execution Context components, lifecycle, and internal mechanisms.
