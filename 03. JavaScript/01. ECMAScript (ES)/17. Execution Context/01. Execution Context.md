...
- It refers to the environment where your code runs.
...
- When execution completes, the reference is popped — and the EC is discarded.
...
**Execution Context does get destroyed — but it isn’t considered a phase. Once execution finishes, the context is popped from the stack. Only creation & execution are meaningful phases.**
...
- `Within a single execution context, code is executed sequentially`, following the order in which it appears in the code. This means that statements within the same function are executed in the order they are written, from top to bottom.
- There's no reordering based on LIFO/FILO principles within the same execution context.
