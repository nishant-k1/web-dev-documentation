...
> 📖 **For detailed operator information:** See [01. Operators](./01.%20Operators.md)
> 📖 **For precedence details:** See [02. Operators Precedence](./02.%20Operators%20Precedence.md)
> 📖 **For operator coercion:** See [03. Operator Coercion](./03.%20Operator%20Coercion.md)
...
- `[] + []` → `""` (empty string)
- `{} + []` → `0` (in some contexts) or `"[object Object]"` (in others)
- `[] + {}` → `"[object Object]"`
- `{} + {}` → `"[object Object][object Object]"` or `NaN` (in some contexts)

**Explanation:**

- Arrays convert to strings via `toString()`: `[].toString()` → `""`
- Objects convert to strings: `{}.toString()` → `"[object Object]"`
...
- `"" == 0` → `true` (empty string coerces to 0)
- `"" === 0` → `false` (strict)
- `" " == 0` → `false` (space string coerces to 0, but " " != "")
...
- `0 && 1` → `0` (first falsy)
- `1 && 0` → `0` (second operand, first is truthy)
- `0 || 1` → `1` (first truthy)
- `1 || 0` → `1` (first truthy)
- `null || undefined || "default"` → `"default"` (first truthy)
...
- `true && false || true` → `true` (AND first, then OR)
- `true || false && true` → `true` (OR short-circuits)
- `null || "A" ?? "B"` → `"A"` (OR has higher precedence than ??)
...
- Non-empty array → comma-separated string → NaN
...
- `typeof []` → `"object"`
- `typeof {}` → `"object"`
- `typeof typeof 1` → `"string"` (typeof returns string)
...
- `sum` → `6`
- `sum2` → `"0123"` (string concatenation!)
...
- `+obj` → `42`
- `String(obj)` → `"hello"`
- `obj + ""` → `"default"` (hint is "default" for `+` with string)
...
1. **Coercion Rules:**
   ...
   - `-`, `*`, `/` → numeric conversion
   - Arrays/objects → string via `toString()`
...
> - [07. Common Mistakes and Best Practices](./07.%20Common%20Mistakes%20and%20Best%20Practices.md) - Best practices
