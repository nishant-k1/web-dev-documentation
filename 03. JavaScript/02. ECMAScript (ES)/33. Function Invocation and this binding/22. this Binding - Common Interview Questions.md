...
> 📖 **For detailed explanations:** See the specific topic files in this folder.
...
- `method()` → `undefined` (default binding, lost context)
...
- `Person("John")` → Creates `window.name = "John"` (non-strict) or error (strict)
- `new Person("John")` → Creates instance with `name: "John"`
...
- `obj.arrow()` → `undefined` (lexical binding from global scope)
...
- `person.instanceMethod()` → `"John"` (`this` = instance)
- `Person.staticMethod()` → `[Function: Person]` (`this` = class)
...
> - [17. Common Mistakes and Best Practices.md](./16.%20Common%20Mistakes%20and%20Best%20Practices.md) - Best practices
