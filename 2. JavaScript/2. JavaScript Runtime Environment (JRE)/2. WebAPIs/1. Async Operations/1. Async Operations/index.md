...
- So you need a way to be notified when the result is ready — this is handled using callbacks or promises.
...
1. 🔁 A callback or .then() in a promise is used to handle the result of an async operation when it finishes.
2. They are not responsible for making the operation async — they’re just notification handlers.
...
**Async Web APIs are not asynchronous by themselves — they are made to behave asynchronously through the event loop and task queue system.**

Web APIs are async because of the Event Loop — not because they are inherently special.
...
🎯 It pushes the callback or .then() function to the appropriate queue — either the macrotask queue or microtask queue — depending on the API used.
...
localStorage.getItem() — synchronous (can be slow if misused)

alert() — synchronous and blocking UI (not time-taking computationally)

XMLHttpRequest.open(..., false) — synchronous XHR — deprecated, strongly discouraged
