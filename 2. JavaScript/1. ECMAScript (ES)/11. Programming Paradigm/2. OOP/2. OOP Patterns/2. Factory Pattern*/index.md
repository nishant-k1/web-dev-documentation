# 🏭 Factory Pattern in JavaScript (Interview Perspective)

The **Factory Pattern** is a creational design pattern where a **function (factory)** is used to **create and return new objects**, without using the `new` keyword or constructor functions.
...
## 🧪 Comparison: Factory vs Constructor

| Feature          | Factory Pattern           | Constructor Pattern       |
| ---------------- | ------------------------- | ------------------------- |
| Uses `new`?      | ❌ No                     | ✅ Yes                    |
| Uses `this`?     | ❌ No                     | ✅ Yes                    |
| Return value?    | ✅ Explicit (`return {}`) | 🚫 Implicit (`this`)      |
| Prototypes used? | ❌ Usually not            | ✅ Yes (memory optimized) |
| Private data?    | ✅ Easy via closure       | ❌ Not built-in           |
| Memory usage     | ⚠️ Higher per object      | ✅ Shared via prototype   |
...
function createUser(name) {
  return {
    name,
    greet: () => `Hi ${name}`, // ❌ recreated every time
  };
}
```

✅ Instead, extract shared methods to a separate object or use prototype if needed.
