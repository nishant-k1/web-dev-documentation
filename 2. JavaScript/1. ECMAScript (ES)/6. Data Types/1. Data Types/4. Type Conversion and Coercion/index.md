...
3. **Boolean Coercion**
   ...
   - JavaScript automatically converts values to `true` or `false` in contexts that expect a Boolean, such as `if statements`, `logical operators`, etc.
     ...
     6. `null` – Represents a null or non-existent value.
     7. `undefined` – A variable that has been declared but not assigned a value.
     8. `NaN` – Not a Number (result of invalid mathematical operations).
...
4. **Logical Operators**
   The && (AND) and || (OR) operators perform type coercion based on the operands' `truthiness` or `falsiness`.
