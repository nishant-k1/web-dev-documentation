...
- An Execution Context is created whenever a function is invoked (including arrow functions), or when the global code is run (i.e., the first time the JavaScript engine starts executing the code).
...
## 🔥 Critical Concept: Each Function CALL Creates an INDEPENDENT Execution Context (Even for the same function definition/declaration)

**Same function definition ≠ Same execution context**
...
function createCounter() {
  let count = 0; // ← This variable is created FRESH on every call
  return {
    increment: () => count++,
    getCount: () => count,
  };
...
counter1.increment(); // counter1's count: 0 → 1
counter1.increment(); // counter1's count: 1 → 2
counter2.increment(); // counter2's count: 0 → 1
...
```
┌─────────────────────────────────┐
│  createCounter() - Call 1       │
│  ┌─────────────────────┐        │
│  │ count = 0 → 1 → 2   │ ← EC1  │
│  └─────────────────────┘        │
│         ↑                       │
│    counter1 methods             │
│    reference this count         │
└─────────────────────────────────┘

┌─────────────────────────────────┐
│  createCounter() - Call 2       │
│  ┌─────────────────────┐        │
│  │ count = 0 → 1       │ ← EC2  │
│  └─────────────────────┘        │
│         ↑                       │
...
```javascript
let sharedCount = 0; // ← Outside the function (shared scope)
...
- In JavaScript, the call stack manages `stacks of execution contexts`.
- A new execution context gets created when:
  - The JavaScript engine starts running (Global Execution Context)
  - A function is called (Function Execution Context)
  - `eval()` is executed (Eval Execution Context)
  - A module is imported (Module Execution Context)
...
- You cannot directly see or manipulate the call stack in your code, but understanding how it works is crucial to understanding how JavaScript executes code.
  <!-- Image: Call Stack visualization showing execution contexts stacked -->
